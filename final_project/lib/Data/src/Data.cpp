...
bool Data_updateStateHistory(rmsClass& rmsClassArg, char dataFileName[]){
    ...
    uint32_t timeFrame = SMS_HW_WQ;  // Example: 20 seconds
    ...
    // Reset the file cursor to the first row of data (excluding the header row)
    //TODO: this seems to not be working: it stilll reads the first row
    dataFile.seek(1);
    ...
    while (dataFile.available()) {
        ...
        if ((currentEPochTimestamp - rowTimestamp) <= timeFrame) {
            ...
            //set appropriate state history count
